...
// U. Brandes, On variants of shortest-path betweenness centrality and their generic
// computation, Social Networks (2008)
...
void getLoadListOfGraph(Graph& G, vector<double>& Load) {
    ...
    for (Node s = 0; s < N; ++s) {
        ...
        while (!S.empty()) {
            Node w = S.top();
            S.pop();
            for (Node v : Pred[w]) delta[v] += (delta[w] / Pred[w].size());
