...
Graph::Graph(const Node numVertices = 0)
:   adjacencyList(numVertices), numEdges(0) {
...
void Graph::setNodesNum(const Node numNode){
    adjacencyList.resize(numNode);
}
...
void Graph::removeAllEdgesOf(const Node N){
...
void Graph::importFromListOfLinks(string filename){
    ...
    if (fin.is_open()){
        while (fin >> N1 >> N2) {
            if(max(N1, N2)+1 > getSize()){
                setNodesNum(max(N1, N2)+1);
            }
...
Node Graph::getSize() const {
    return adjacencyList.size();
}

Node Graph::getNumFriendsOf(const Node v) const {
    return adjacencyList[v].size();
}

Node Graph::getNumEdges() const {
    return numEdges;
}

const vector<Node>& Graph::getFriendsOf(const Node v) const {
    return adjacencyList[v];
}

const vector<vector<Node>>& Graph::getAdjacencyList() const {
