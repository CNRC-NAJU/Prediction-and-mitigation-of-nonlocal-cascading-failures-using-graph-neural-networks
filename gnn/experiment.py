...
from gnn import schedular
from gnn.data import MotterLaiData
from gnn.earlystop import EarlyStop
from gnn.hyperparameter import HyperParameter
from gnn.metric import Metric, MetricName
from gnn.models import count_trainable_param, get_gnn
from gnn.timer import Timer
from gnn.utils import configure_logger
...
class Experiment:
    ...
    def run(self, max_epoch: int | None = None) -> None:
    ...
    # =================================== Utility ====================================
    def _get_range(
        self, iterable: Iterable[Any], desc: str | None = None
    ) -> Iterable[Any]:
        """
    ...
    def plot_metric(
        ...
        fig_ax: tuple[fig.Figure, plt.Axes] | None = None,
        schedular_list: list[int] | None = None,
        save: bool = False,
        ...
        # Save plot
        if save:
            fig.savefig(str(exp_dir / f"{metric_name.value}.png"), facecolor="w")
        return fig, ax
