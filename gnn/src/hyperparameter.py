...
def get_args(options: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="normalize: [degree/max_degree]\n"
        "log: [log10(degree)]\n"
        "inverse: [1/degree]\n"
    ...
    parser.add_argument(
        "--networkwise",
        action="store_true",
        help="If this flag is on, metrics are calculated networkwise and then averaged",
    )
    parser.add_argument(
        "--loss",
        default="MAE",
...
class HyperParameter:
    ...
    seed: Optional[int] = field(compare=False)
    ...
    exp_id: Optional[int] = field(compare=False)
    ...
    def save(self) -> None:
        ...
        with open(exp_dir / "hyper_parameter.json", "w") as f:
            json.dump(attributes_dict, f, indent=4)

    @classmethod
    def load(cls, exp_id: int, root_dir: Union[str, Path] = BASE_DIR / "experiment"):
